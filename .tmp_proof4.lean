...
private def walkOfList (d : ℕ) :
  ...
  | a :: b :: tl, _, hchain => by
      -- `Chain'` is defeq to `List.IsChain`
...
private lemma walkTime_dropUntil_le (d : ℕ) (w : Weights d) {u v : Zd d}
  ...
  have htime : walkTime d w p =
    ...
    rw [hs]
    simpa using (walkTime_append d w (p.takeUntil x hx) (p.dropUntil x hx))
...
private lemma walkTime_bypass_le (d : ℕ) (w : Weights d) {u v : Zd d}
  ...
  | cons ha p ih =>
      ...
      ·
        -- simplify the goal to the dropUntil branch
        simp [SimpleGraph.Walk.bypass, p', hs]
        have h1 : walkTime d w (p'.dropUntil u hs) ≤ walkTime d w p' :=
          walkTime_dropUntil_le d w (p := p') (x := u) hs
        have h2 : walkTime d w p' ≤ walkTime d w p := by
          simpa [p'] using ih
      ...
      ·
        -- simplify the goal to the cons branch
        simp [SimpleGraph.Walk.bypass, p', hs]
        have h2 : walkTime d w p' ≤ walkTime d w p := by
          simpa [p'] using ih
        -- compare after adding the first edge
        simpa [walkTime_cons, add_comm, add_left_comm, add_assoc] using (add_le_add_left h2 (w u v))
