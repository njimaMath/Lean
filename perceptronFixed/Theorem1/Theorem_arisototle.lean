...
lemma E_sq_le_poly (u : ℝ) : (Theorem1.E u)^2 ≤ 2 * u^2 + 4 := by
  by_cases hu : u < 0;
  · -- Since $u < 0$, we have $E(u) = \frac{\varphi(u)}{\Phibar(u)} \leq \frac{\varphi(u)}{\Phibar(0)} = \frac{\varphi(u)}{1/2} = 2\varphi(u)$.
    have h_E_le_2phi : E u ≤ 2 * φ u := by
      -- Since Φbar(u) ≥ Φbar(0) = 1/2 for u < 0, we have E(u) = φ(u) / Φbar(u) ≤ φ(u) / (1/2) = 2 * φ(u).
      have h_E_le_2phi : E u ≤ φ u / (1 / 2) := by
        have h_E_le_2phi : Φbar u ≥ Φbar 0 := by
          apply_rules [ MeasureTheory.setIntegral_mono_set ];
          · exact MeasureTheory.Integrable.integrableOn ( by exact MeasureTheory.integrable_of_integral_eq_one ( by rw [ show DecreasingG.φ = fun u => Real.exp ( - ( u ^ 2 ) / 2 ) / Real.sqrt ( 2 * Real.pi ) from funext fun _ => rfl ] ; exact by rw [ MeasureTheory.integral_div, div_eq_iff ( by positivity ) ] ; simpa [ div_eq_inv_mul ] using integral_gaussian ( 1 / 2 ) ) );
          · exact Filter.Eventually.of_forall fun x => by exact div_nonneg ( Real.exp_nonneg _ ) ( Real.sqrt_nonneg _ ) ;
          · exact MeasureTheory.ae_of_all _ fun x hx => le_trans hu.le hx;
        have h_E_le_2phi : Theorem1.Φbar 0 = 1 / 2 := by
          have h_Φbar_zero : ∫ x in Set.Ici 0, (Real.exp (-(x ^ 2) / 2)) / Real.sqrt (2 * Real.pi) = 1 / 2 := by
    ...
    have h_phi_le_one : φ u ≤ 1 / Real.sqrt (2 * Real.pi) := by
      exact div_le_div_of_nonneg_right ( Real.exp_le_one_iff.mpr <| by nlinarith ) ( by positivity ) |> le_trans <| by ring_nf; norm_num;
    -- Since $\frac{2}{\sqrt{2\pi}} < 1$, we have $2\varphi(u) < 1$.
    have h_2phi_lt_one : 2 * φ u < 1 := by
      exact lt_of_le_of_lt ( mul_le_mul_of_nonneg_left h_phi_le_one zero_le_two ) ( by rw [ mul_one_div, div_lt_iff₀ ] <;> nlinarith only [ Real.pi_gt_three, Real.sqrt_nonneg ( 2 * Real.pi ), Real.sq_sqrt ( show 0 ≤ 2 * Real.pi by positivity ) ] );
    nlinarith [ show 0 ≤ Theorem1.E u from div_nonneg ( div_nonneg ( Real.exp_nonneg _ ) ( Real.sqrt_nonneg _ ) ) ( MeasureTheory.integral_nonneg fun _ => div_nonneg ( Real.exp_nonneg _ ) ( Real.sqrt_nonneg _ ) ) ];
  · -- Since `u ≥ 0`, we have `E u ≤ u + d 0`.
    have h_E_le : Theorem1.E u ≤ u + DecreasingG.d 0 := by
      -- Since `u ≥ 0`, we have `d u ≤ d 0` by `DecreasingG.d_le_d0_of_nonneg`.
      have h_d_le : DecreasingG.d u ≤ DecreasingG.d 0 := by
        apply DecreasingG.d_le_d0_of_nonneg; exact le_of_not_gt hu
      generalize_proofs at *; (
      unfold DecreasingG.d at *; linarith;)
    generalize_proofs at *; (
    -- Since `d 0 = sqrt(2 / pi)`, we have `E u ≤ u + sqrt(2 / pi)`.
    have h_E_le_sqrt : Theorem1.E u ≤ u + Real.sqrt (2 / Real.pi) := by
      exact h_E_le.trans ( by rw [ DecreasingG.d0_eq_sqrt_two_div_pi ] )
    generalize_proofs at *; (
    exact le_trans ( pow_le_pow_left₀ ( by exact div_nonneg ( div_nonneg ( Real.exp_nonneg _ ) ( Real.sqrt_nonneg _ ) ) ( MeasureTheory.integral_nonneg fun _ => div_nonneg ( Real.exp_nonneg _ ) ( Real.sqrt_nonneg _ ) ) ) h_E_le_sqrt 2 ) ( by nlinarith [ Real.sqrt_nonneg ( 2 / Real.pi ), Real.mul_self_sqrt ( show 0 ≤ 2 / Real.pi by positivity ), show 2 / Real.pi ≤ 1 by rw [ div_le_iff₀ <| by positivity ] ; linarith [ Real.pi_gt_three ] ] )))
...
lemma integral_E_sq_continuousAt (κ : ℝ) (q₀ : ℝ) (hq₀ : q₀ < 1) :
    ContinuousAt (fun q => Theorem1.Expect (fun z => (Theorem1.E (Theorem1.U κ q z)) ^ 2)) q₀ := by
      ...
      use fun z => 2 * ( 1 / Real.sqrt ( 1 - ( q₀ + ( 1 - q₀ ) / 2 ) ) * ( |κ| + |z| ) ) ^ 2 + 4;
      · refine' Filter.eventually_of_mem ( Iio_mem_nhds hq₀ ) fun q hq => Measurable.aestronglyMeasurable _;
        apply_rules [ Measurable.pow_const, Measurable.div, Measurable.mul, measurable_const, measurable_id ];
        · exact Measurable.exp ( Measurable.div_const ( Measurable.neg ( Measurable.pow_const ( by exact Measurable.div_const ( by exact Measurable.sub ( measurable_const ) ( measurable_const.mul measurable_id' ) ) _ ) _ ) ) _ );
        · -- The function $U(\kappa, q, z)$ is measurable since it is a composition of measurable functions.
          have h_U_measurable : Measurable (fun z => (κ - Real.sqrt q * z) / Real.sqrt (1 - q)) := by
            exact Measurable.mul ( Measurable.sub measurable_const ( measurable_const.mul measurable_id' ) ) measurable_const;
          -- The function Φbar is measurable since it is the integral of a measurable function.
          have h_Φbar_measurable : Measurable (fun u => ∫ x in Set.Ici u, φ x) := by
            have h_Φbar_measurable : Measurable (fun u => ∫ x in Set.Ioi u, φ x) := by
              have h_Φbar_measurable : Measurable (fun u => ∫ x in Set.Iic u, φ x) := by
                apply_rules [ Monotone.measurable, measurable_const ];
                refine' fun u v huv => MeasureTheory.setIntegral_mono_set _ _ _;
                · exact MeasureTheory.Integrable.integrableOn ( by exact MeasureTheory.integrable_of_integral_eq_one ( by rw [ show Theorem1.φ = fun u => Real.exp ( - ( u ^ 2 ) / 2 ) / Real.sqrt ( 2 * Real.pi ) from funext fun u => rfl ] ; rw [ MeasureTheory.integral_div, div_eq_iff ( by positivity ) ] ; simpa [ div_eq_inv_mul ] using integral_gaussian ( 1 / 2 ) ) );
                · exact Filter.Eventually.of_forall fun x => div_nonneg ( Real.exp_nonneg _ ) ( Real.sqrt_nonneg _ );
                · exact MeasureTheory.ae_of_all _ fun x hx => le_trans hx huv;
              have h_Φbar_measurable : ∀ u, ∫ x in Set.Ioi u, φ x = (∫ x in Set.univ, φ x) - (∫ x in Set.Iic u, φ x) := by
                intro u; rw [ ← MeasureTheory.integral_diff ] <;> norm_num;
                · rcongr x ; aesop;
                · exact MeasureTheory.integrable_of_integral_eq_one ( by rw [ show Theorem1.φ = fun x => Real.exp ( -x ^ 2 / 2 ) / Real.sqrt ( 2 * Real.pi ) by ext; rfl ] ; rw [ MeasureTheory.integral_div, div_eq_iff ( by positivity ) ] ; simpa [ div_eq_inv_mul ] using integral_gaussian ( 1 / 2 ) );
              simpa only [ h_Φbar_measurable ] using measurable_const.sub ‹Measurable fun u => ∫ x in Set.Iic u, Theorem1.φ x›;
            simpa only [ MeasureTheory.integral_Ici_eq_integral_Ioi ] using h_Φbar_measurable;
          exact h_Φbar_measurable.comp h_U_measurable;
      · refine' Filter.eventually_of_mem ( Ioo_mem_nhds ( show q₀ - ( 1 - q₀ ) / 2 < q₀ by linarith ) ( show q₀ < q₀ + ( 1 - q₀ ) / 2 by linarith ) ) fun q hq => Filter.Eventually.of_forall fun z => _;
        -- By definition of $U$, we know that $|U κ q z| ≤ M (|κ| + |z|)$ for some $M$ depending on $q₀$.
        have h_U_bound : |Theorem1.U κ q z| ≤ (1 / Real.sqrt (1 - (q₀ + (1 - q₀) / 2))) * (|κ| + |z|) := by
          unfold Theorem1.U;
          rw [ abs_div, abs_of_nonneg ( Real.sqrt_nonneg _ ) ];
          field_simp;
          gcongr;
          · exact Real.sqrt_pos.mpr ( by linarith [ hq.1, hq.2 ] );
      ...
      · -- The function $2 * (1 / \sqrt{1 - (q₀ + (1 - q₀) / 2)} * (|κ| + |z|))^2 + 4$ is integrable because it is a polynomial in $|z|$.
        have h_integrable : MeasureTheory.Integrable (fun z => |z|^2) Theorem1.γ := by
          have h_gauss_moment : ∫ z, z ^ 2 ∂Theorem1.γ = 1 := by
            have h_gauss_integrable : ∫ z, z^2 ∂(ProbabilityTheory.gaussianReal 0 1) = 1 := by
              have h_gauss_integrable : ∫ z, z^2 * (Real.exp (-z^2 / 2)) / Real.sqrt (2 * Real.pi) = 1 := by
                have h_gauss_integrable : ∫ z, z^2 * (Real.exp (-z^2 / 2)) = Real.sqrt (2 * Real.pi) := by
                  have := @integral_rpow_mul_exp_neg_mul_rpow;
                  -- We'll use the fact that $\int_{-\infty}^{\infty} z^2 e^{-z^2/2} \, dz$ is the Gaussian integral, which evaluates to $\sqrt{2\pi}$.
                  have h_gauss_integral : ∫ z in Set.Ioi 0, z^2 * Real.exp (-z^2 / 2) = Real.sqrt (2 * Real.pi) / 2 := by
                  ...
                  · ring;
                rw [ MeasureTheory.integral_div, h_gauss_integrable, div_self ( by positivity ) ]
              convert h_gauss_integrable using 1;
              norm_num [ ProbabilityTheory.gaussianReal ];
            ...
            exact h_gauss_integrable;
          simpa using MeasureTheory.integrable_of_integral_eq_one h_gauss_moment;
        ...
        · ring_nf;
          ...
          · refine' MeasureTheory.Integrable.mono' _ _ _;
            exacts [ fun z => |z| ^ 2 + 1, by exact MeasureTheory.Integrable.add h_integrable ( MeasureTheory.integrable_const _ ), by exact measurable_norm.aestronglyMeasurable, Filter.Eventually.of_forall fun z => by simpa using abs_le.mpr ⟨ by nlinarith, by nlinarith ⟩ ];
      ...
      · refine' Filter.Eventually.of_forall fun z => _;
        ...
        · refine' Filter.Tendsto.div _ _ _ <;> norm_num [ Theorem1.U ];
          ...
          · exact Filter.Tendsto.sqrt ( tendsto_const_nhds.sub Filter.tendsto_id );
    ...
    have h_E_neg : DecreasingG.E u ≤ 2 * φ u := by
      have h_E_neg : DecreasingG.E u ≤ φ u / (1 / 2) := by
        have h_E_neg : DecreasingG.Φbar u ≥ 1 / 2 := by
          -- Since $u < 0$, we have $\Phibar(u) = \int_{u}^{\infty} \varphi(x) \, dx \geq \int_{0}^{\infty} \varphi(x) \, dx = \Phibar(0)$.
          ...
          -- Since $\int_{0}^{\infty} \varphi(x) \, dx = \frac{1}{2}$, we have $\Phibar(0) = \frac{1}{2}$.
      ...
      linarith;
    -- Since $\varphi(u) \leq \frac{1}{\sqrt{2\pi}}$, we have $2\varphi(u) \leq \frac{2}{\sqrt{2\pi}} < 2$.
...
lemma Theorem1.E_asymp_atTop : Filter.Tendsto (fun u => Theorem1.E u / u) Filter.atTop (nhds 1) := by
  -- We'll use the fact that $E(u) = u + d(u)$ and $d(u) \to 0$ as $u \to \infty$.
  have h_E_div_u : Filter.Tendsto (fun u => (u + DecreasingG.d u) / u) Filter.atTop (𝓝 1) := by
    -- Since $d(u)$ is bounded above by $d(0)$ for $u \geq 0$, we have $|d(u) / u| \leq d(0) / u$.
...
lemma Theorem1.limit_one_sub_q_mul_U_sq (κ z : ℝ) : Filter.Tendsto (fun q => (1 - q) * (Theorem1.U κ q z)^2) (nhdsWithin 1 (Set.Iio 1)) (nhds ((κ - z)^2)) := by
  ...
  -- Cancel out the common term $(1 - q)$ in the numerator and denominator.
...
lemma Theorem1.tendsto_E_atBot : Filter.Tendsto Theorem1.E Filter.atBot (nhds 0) := by
  -- Since $E = \frac{\varphi}{\bar{\Phi}}$, and $\varphi(u) \to 0$ as $u \to -\infty$, we have $E(u) \to 0$ as $u \to -\infty$.
  have hE_tendsto_zero : Filter.Tendsto (fun u => DecreasingG.φ u / DecreasingG.Φbar u) Filter.atBot (nhds 0) := by
    ...
    -- The denominator $\int_{u}^{\infty} \varphi(x) \, dx$ converges to 1 as $u \to -\infty$.
...
lemma Theorem1.tendsto_U_atTop (κ z : ℝ) (h : z < κ) :
    Filter.Tendsto (fun q => Theorem1.U κ q z) (nhdsWithin 1 (Set.Iio 1)) Filter.atTop := by
      -- We'll use the fact that as $q \to 1^-$, $\sqrt{1 - q} \to 0^+$ and $(κ - \sqrt{q} z) \to (κ - z)$.
      ...
      -- We'll use the fact that as $q \to 1^-$, $(κ - \sqrt{q} z) \to (κ - z)$.
...
lemma Theorem1.tendsto_U_atBot (κ z : ℝ) (h : κ < z) :
    Filter.Tendsto (fun q => Theorem1.U κ q z) (nhdsWithin 1 (Set.Iio 1)) Filter.atBot := by
      ...
      -- We can use the fact that $(κ - \sqrt{q} z) \to κ - z$ as $q \to 1^-$.
...
lemma Theorem1.integrand_limit_of_eq (κ z : ℝ) (h : κ = z) :
    Filter.Tendsto (fun q => (1 - q) * (Theorem1.E (Theorem1.U κ q z))^2) (nhdsWithin 1 (Set.Iio 1)) (nhds 0) := by
      -- Since $U$ approaches $0$ as $q$ approaches $1$, and $E$ is continuous, $E(U)$ approaches $E(0)$.
      have h_E_lim : Filter.Tendsto (fun q => Theorem1.E (Theorem1.U κ q z)) (nhdsWithin 1 (Set.Iio 1)) (nhds (Theorem1.E 0)) := by
        have h_E_lim : Filter.Tendsto (fun q => Theorem1.U κ q z) (nhdsWithin 1 (Set.Iio 1)) (nhds 0) := by
          -- Use the fact that $U \to 0$ as $q \to 1$ by simplifying the expression.
          have h_U_zero : Filter.Tendsto (fun q => (1 - Real.sqrt q) / Real.sqrt (1 - q)) (nhdsWithin 1 (Set.Iio 1)) (nhds 0) := by
            -- Use the fact that $\frac{1 - \sqrt{q}}{\sqrt{1 - q}} = \frac{\sqrt{1 - q}}{1 + \sqrt{q}}$.
...
lemma Theorem1.tendsto_U_zero_of_eq (κ z : ℝ) (h : κ = z) :
    Filter.Tendsto (fun q => Theorem1.U κ q z) (nhdsWithin 1 (Set.Iio 1)) (nhds 0) := by
      -- Let $y = \sqrt{q}$. Then as $q \to 1$, $y \to 1$.
...
lemma Theorem1.integrand_bound (κ : ℝ) : Filter.Eventually (fun q => ∀ z, (1 - q) * (Theorem1.E (Theorem1.U κ q z))^2 ≤ 4 * (κ^2 + z^2) + 10) (nhdsWithin 1 (Set.Iio 1)) := by
  ...
  -- Substitute $U(\kappa, q, z) = \frac{\kappa - \sqrt{q} z}{\sqrt{1-q}}$ into the inequality.
  ...
  · -- The function $4 * (κ^2 + z^2) + 100$ is a polynomial in $z$, and polynomials are integrable with respect to the Gaussian measure.
    have h_poly_integrable : MeasureTheory.Integrable (fun z => z^2) Theorem1.γ := by
      have h_gauss_moment : ∫ z, z^2 * (Real.exp (-z^2 / 2)) ∂MeasureTheory.volume = Real.sqrt (2 * Real.pi) := by
        have := @integral_rpow_mul_exp_neg_mul_rpow;
        -- We'll use the fact that $\int_{-\infty}^{\infty} z^2 e^{-z^2/2} \, dz$ can be computed using polar coordinates.
        ...
        -- Since $z^2 e^{-z^2/2}$ is an even function, we can double the integral over $(0, \infty)$ to get the integral over $(-\infty, \infty)$.
        have h_even : ∫ z in Set.Iic 0, z^2 * Real.exp (-z^2 / 2) = Real.sqrt (2 * Real.pi) / 2 := by
...
lemma bridge_g_eq (u : ℝ) : MillsBlueprint.Proof.g u = DecreasingG.g u := by
  ...
  · rw [ show ∫ x in Set.Ici u, DecreasingG.φ x = 1 - ∫ x in Set.Iic u, DecreasingG.φ x from ?_ ];
    -- The integral of the Gaussian density over the entire real line is 1.
...
lemma bridge_E_eq (u : ℝ) : MillsBlueprint.Proof.E u = DecreasingG.E u := by
  -- By definition of $γ$, we know that $\int_{-\infty}^{\infty} \frac{1}{\sqrt{2 \pi}} e^{-z^2 / 2} \, dz = 1$.
  have h_gauss_total : ∫ z, (1 / Real.sqrt (2 * Real.pi)) * Real.exp (-z^2 / 2) = 1 := by
    rw [ MeasureTheory.integral_const_mul ];
    rw [ one_div, inv_mul_eq_div, div_eq_iff ( by positivity ) ];
    simpa [ div_eq_inv_mul ] using integral_gaussian ( 1 / 2 );
  -- By definition of $Φbar$, we know that $\int_{-\infty}^{u} \frac{1}{\sqrt{2 \pi}} e^{-z^2 / 2} \, dz = 1 - \int_{u}^{\infty} \frac{1}{\sqrt{2 \pi}} e^{-z^2 / 2} \, dz$.
...
lemma bridge_B_eq (κ q : ℝ) :
  ...
  -- By definition of `Theorem1.B` and `MillsBlueprint.Proof.B`, we can see that they are equal.
...
lemma deriv_B_eq (κ : ℝ) (q : ℝ) (hq : q ∈ Set.Ioo 0 1) :
  ...
  -- Apply the result from` MillsBlueprint.Proof.deriv_B_eq_expect_g` with `P = Theorem1.γ`, `Z = id`, `t = q`.
...
lemma g0_bound : DecreasingG.g 0 < -1 / 18 := by
  -- Use `g0_eq` to rewrite the goal in terms of `Real.pi`.
...
lemma arithmetic_bound (p : ℝ) (hp : 1/2 ≤ p) (hp1 : p ≤ 1) :
  ...
  -- By definition of $g(0)$, we know that $g(0) < -1/18$.
...
lemma gamma_U_nonneg_ge_half (κ : ℝ) (hκ : 0 ≤ κ) (q : ℝ) (hq : q ∈ Set.Ioo 0 1) :
  ...
  -- The set `{z | U(κ, q, z) ≥ 0}` corresponds to `z ≤ κ / Real.sqrt q`.
  have h_set_eq : {z | Theorem1.U κ q z ≥ 0} = Set.Iic (κ / Real.sqrt q) := by
    ...
    grind;
  -- The measure of the set `{z | z ≤ κ / Real.sqrt q}` under the standard Gaussian is `Φ(κ / Real.sqrt q)`.
  have h_measure_eq : (Theorem1.γ (Set.Iic (κ / Real.sqrt q))).toReal = ∫ x in Set.Iic (κ / Real.sqrt q), (Real.exp (-x^2 / 2)) / Real.sqrt (2 * Real.pi) := by
    ...
    · exact Filter.Eventually.of_forall fun x => by positivity;
    · exact Continuous.aestronglyMeasurable ( by continuity );
  -- Since `κ ≥ 0` and `q > 0`, `κ / Real.sqrt q ≥ 0`. The measure of `{z | z ≤ κ / Real.sqrt q}` under the standard Gaussian is `Φ(κ / Real.sqrt q)`.
  ...
  -- The integral of the Gaussian density over $(-\infty, 0]$ is equal to $1/2$.
...
lemma g_bound_poly : ∃ C, ∀ u, |DecreasingG.g u| ≤ C * (1 + |u|^4) := by
  -- By definition of $g$, we know that $|g(u)| \leq (E(u))^2 * (3 * (E(u))^2 + 4 * |u| * E(u) + |u|^2 + 2)$.
  ...
  -- By definition of $E$, we know that $|E(u)| \leq |u| + C_mills$.
  ...
  -- Substitute `|E u| ≤ |u| + C` into the bound for `|g(u)|`.
  ...
  have h_subst : ∀ u : ℝ, |(DecreasingG.E u) ^ 2 * (3 * (DecreasingG.E u) ^ 2 - 4 * u * (DecreasingG.E u) + u ^ 2 - 2)| ≤ (|u| + C) ^ 2 * (3 * (|u| + C) ^ 2 + 4 * |u| * (|u| + C) + |u| ^ 2 + 2) := by
    ...
    · exact le_of_lt ( DecreasingG.E_pos u );
  -- The right-hand side is a polynomial in $|u|$ of degree 4, so it is bounded by some constant times $(1 + |u|^4)$.
...
lemma integrable_g_U (κ : ℝ) (q : ℝ) (hq : q ∈ Set.Ioo 0 1) :
  ...
  -- Use `g_bound_poly` to get `C` such that `|g(u)| ≤ C * (1 + |u|^4)`.
  ...
  · refine' MeasureTheory.Integrable.const_mul _ _;
    refine' MeasureTheory.Integrable.add _ _;
    · norm_num [ Theorem1.γ ];
    · -- The function $|(κ - √q * z) / √(1 - q)|^4$ is a polynomial in $z$, and polynomials are integrable with respect to the Gaussian measure.
...
theorem B_strictAntiOn_Icc (κ : ℝ) (hκ : 0 ≤ κ) :
  ...
  -- First, prove that B(κ, q) is strictly decreasing on [0, 1).
  have h_anti : ∀ q₁ q₂ : ℝ, 0 ≤ q₁ → q₁ < q₂ → q₂ ≤ 1 → Theorem1.B κ q₁ > Theorem1.B κ q₂ := by
    -- Apply the Mean Value Theorem to the interval [q₁, q₂].
    ...
    -- Since the derivative of B is negative on (0,1), we can apply the Mean Value Theorem to any interval [q₁, q₂] within [0,1] to show that B(q₁) > B(q₂).
    intros q₁ q₂ hq₁ hq₂ hq₂_le_one
    by_cases hq₂_eq_one : q₂ = 1;
    · -- Since $B$ is strictly decreasing on $[0,1)$, we have $B(q₁) > B(1)$.
      have h_strict_decr : ∀ q₁ q₂ : ℝ, 0 ≤ q₁ → q₁ < q₂ → q₂ < 1 → Theorem1.B κ q₁ > Theorem1.B κ q₂ := by
        ...
        exact h_diff_pos;
      -- Since $B$ is strictly decreasing on $[0,1)$, we have $B(q₁) > B(1)$ by taking the limit as $q₂$ approaches $1$ from the left.
...
lemma f_continuousOn_Ici (κ α : ℝ) : ContinuousOn (f κ α) (Set.Ici (0 : ℝ)) := by
  ...
  -- The function $f$ is the difference of two continuous functions, hence it is continuous.
  have h_cont : ContinuousOn (fun r => A r) (Set.Ici 0) ∧ ContinuousOn (fun r => α * B κ (P r)) (Set.Ici 0) := by
    -- The function $A(r)$ is continuous because it is a product of continuous functions.
    have hA_cont : ContinuousOn (fun r => A r) (Set.Ici 0) := by
      -- The function $A(r)$ is continuous on $[0, \infty)$ because it is a product of continuous functions.
      apply A_continuousOn_Ici;
    -- Since $B κ$ is continuous on $(-∞, 1)$ and $P$ is continuous on $[0, ∞)$, their composition $B κ ∘ P$ is continuous on $[0, ∞)$.
...
lemma f_zero (κ α : ℝ) : f κ α 0 = -α * (B κ 0) := by
  -- Use `A(0)=0` and `P(0)=0`.
  -- By definition of $f$, we have $f(0) = A(0) - \alpha B(P(0))$.
  simp [Theorem1.f, Theorem1.A, Theorem1.P]

lemma f_zero_neg (κ α : ℝ) (hα : 0 < α) : f κ α 0 < 0 := by
  -- Use `B(0) = (E κ)^2 > 0`.
  -- By definition of $f$, we know that $f κ α 0 = -α * (E κ)^2$.
  have h_f_zero : Theorem1.f κ α 0 = -α * (E κ) ^ 2 := by
    -- By definition of $f$, we know that $f(κ, α, 0) = A(0) - α * B(κ, P(0))$.
    simp [Theorem1.f, Theorem1.A, Theorem1.B, Theorem1.P_zero];
    -- Since $E$ is a constant function, the integral of $E^2$ over the Gaussian measure is just $E^2$ times the integral of $1$ over the Gaussian measure, which is $1$.
    simp [Theorem1.Expect, Theorem1.U]
  generalize_proofs at *; simp_all +decide [ E ] ; (
  -- Since $E κ$ is positive, its square is also positive.
...
lemma tendsto_f_atTop (κ α : ℝ) :
  ...
  have hB_lim : Filter.Tendsto (fun r => B κ (P r)) Filter.atTop (nhds (Cκ κ)) := by
    ...
    have hP_lim : Filter.Tendsto P Filter.atTop (nhds 1) := by
      -- Apply the lemma that states the limit of P(r) as r approaches infinity is 1.
      apply tendsto_P_atTop;
    have hB_cont : Filter.Tendsto (fun q => B κ q) (nhdsWithin 1 (Set.Iio 1)) (nhds (Cκ κ)) := by
      -- Apply the hypothesis that $B$ tends to $C_k(\kappa)$ as $q$ approaches $1$ from the left.
...
lemma exists_root_of_alpha_lt_alpha_c
  ...
  -- Apply the intermediate value theorem to $f$ on the interval $(0, \infty)$.
...
lemma existsUnique_r_of_alpha_lt_alpha_c
  ...
  -- By the existence and uniqueness lemmas, there exists a unique $r \in (0, \infty)$ such that $f(r) = 0$.
  obtain ⟨r, hr⟩ : ∃ r : ℝ, r ∈ Set.Ioi (0 : ℝ) ∧ Theorem1.f κ α r = 0 := by
    -- Apply the lemma exists_root_of_alpha_lt_alpha_c with the given hypotheses.
    apply exists_root_of_alpha_lt_alpha_c κ α hα0 hα hB;
  -- By the uniqueness lemma, if there are two roots $r₁$ and $r₂$ in $(0, \infty)$, then $r₁ = r₂$.
...
lemma existsUnique_solution_of_alpha_lt_alpha_c
  ...
  -- By the existence and uniqueness part of Theorem 1, we can conclude that there exists a unique (q, r) satisfying the system.
  obtain ⟨r, hr⟩ : ∃! r : ℝ, r ∈ Set.Ioi (0 : ℝ) ∧ f κ α r = 0 := by
    -- Apply the lemma `existsUnique_r_of_alpha_lt_alpha_c` with the given hypotheses.
    apply existsUnique_r_of_alpha_lt_alpha_c κ α hα0 hα hB;
  -- Let's obtain the unique r from Theorem 1 and set q = P(r).
  use (P r, r);
  -- By definition of $f$, we know that $f(r) = 0$ implies $r = R(P(r))$.
...
lemma no_solution_of_alpha_ge_alpha_c
  ...
  have h_no_root : ∀ r : ℝ, r ∈ Set.Ioi 0 → Theorem1.f κ α r < 0 := by
    ...
    have h_alpha_B_ge_two_pi : ∀ r ∈ Set.Ioi 0, α * Theorem1.B κ (Theorem1.P r) ≥ (2 : ℝ) / Real.pi := by
      -- Since $B(P(r)) \geq Cκ κ$ for all $r > 0$, we have $\alpha B(P(r)) \geq \alpha Cκ κ$.
    ...
    -- Since $A(r) < \frac{2}{\pi}$ for all $r > 0$, we have $A(r) < \alpha B(P(r))$.
  ...
  -- By definition of $IsSolution$, if there were a solution $r$, then $f(r) = 0$.
  intro h_solution
...
lemma qSol_spec (κ α : ℝ) (hκ : 0 ≤ κ) (hα0 : 0 < α) (hα : α < αc κ) :
  ...
  -- Unpack `sol_spec`.
  -- By definition of `sol`, we know that `qSol κ α hκ hα0 hα = P (rSol κ α hκ hα0 hα)` and `rSol κ α hκ hα0 hα = R κ (qSol κ α hκ hα0 hα) α` follows directly from the definition of `sol`.
...
lemma tendsto_q_of_tendsto_r
  ...
  -- Since $P$ tends to $1$ as its argument tends to infinity, and $r$ tends to infinity, we can apply the continuity of $P$.
  have hP_tendsto : Filter.Tendsto (fun x => Theorem1.P x) Filter.atTop (𝓝 1) := by
    -- Apply the fact that $P$ tends to $1$ as $x$ approaches infinity.
    apply tendsto_P_atTop;
  -- Since $r_n \to \infty$, we can apply the continuity of $P$ to conclude that $P(r_n) \to 1$.
  have hP_r_tendsto : Filter.Tendsto (fun n => Theorem1.P (r n)) Filter.atTop (𝓝 1) := by
    -- Apply the fact that if the inner function tends to infinity, then the composition with a continuous function should tend to the same limit.
    apply Filter.Tendsto.comp hP_tendsto hr;
  simpa only [ ← hq ] using hP_r_tendsto
...
lemma exists_frequently_le_of_not_tendsto_atTop
  ...
  -- By definition of not tending to infinity, there exists some $R$ such that for all $N$, there's an $n \geq N$ with $r_n \leq R$.
  have h_not_tendsto : ¬ Filter.Tendsto r Filter.atTop Filter.atTop → ∃ R, ∀ N, ∃ n ≥ N, r n ≤ R := by
    -- By definition of not tending to infinity, there exists some $R$ such that for all $N$, there's an $n \geq N$ with $r_n \leq R$. This follows directly from the negation of the definition of tendsto atTop.
    simp [Filter.tendsto_atTop_atTop] at hnot ⊢;
    -- By definition of not tending to infinity, there exists some $R$ such that for all $N$, there's an $n \geq N$ with $r_n \leq R$. This follows directly from the negation of the definition of tendsto atTop. Use this fact.
    intros x hx; exact ⟨x, fun N => by obtain ⟨n, hn₁, hn₂⟩ := hx N; exact ⟨n, hn₁, le_of_lt hn₂⟩⟩;
  -- By definition of frequently, if for every N there exists an n ≥ N with r n ≤ R, then there are infinitely many such n.
...
lemma solution_at_alpha_c_of_bounded_subseq
  ...
  -- Since α equals αc, the fixed point system doesn't have a solution, leading to a contradiction.
  exfalso;
  -- If the sequence of rSol κ (α n) hκ ⋯ ⋯ is bounded, then there exists a subsequence that converges to some limit rStar.
  ...
  -- Taking the limit of both sides of the equation $rSol κ (α n) hκ (hα n).left (hα n).right = α n * B κ (qSol κ (α n) hκ (hα n).left (hα n).right) / (1 - qSol κ (α n) hκ (hα n).left (hα n).right)^2$ as $n \to \infty$, we get $rStar = αc κ * B κ (P rStar) / (1 - P rStar)^2$.
  ...
  -- Since $rStar$ is a solution to the fixed point system when $\alpha = \alpha_c$, this contradicts the fact that there are no solutions for $\alpha \geq \alpha_c$.
...
theorem theorem_second_main_seq
  ...
  intro h_contra
  generalize_proofs at *;
  -- By contradiction, assume there exists a subsequence of $rSol$ that is bounded. Then there exists $R$ such that $rSol n \leq R$ for infinitely many $n$.
  obtain ⟨R, hR⟩ : ∃ R, ∃ᶠ n in Filter.atTop, Theorem1.rSol κ (α n) hκ (hα n).left (hα n).right ≤ R := by
  ...
  generalize_proofs at *;
  -- By the properties of the solution, if there exists a subsequence of $rSol$ that is bounded, then there exists a solution at $\alpha = \alpha_c(\kappa)$.
  have h_solution_at_alpha_c : ∃ qStar rStar : ℝ, Theorem1.IsSolution κ (Theorem1.αc κ) qStar rStar := by
