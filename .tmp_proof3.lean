...
private def walkOfList (d : ℕ) :
  ...
  | a :: b :: tl, _, hchain => by
      have hchain' : List.IsChain (IsNN (d := d)) (a :: b :: tl) := by
        simpa [List.IsChain] using hchain
      have hab : (NNGraph d).Adj a b := (List.isChain_cons_cons.1 hchain').1
      have htail : List.IsChain (IsNN (d := d)) (b :: tl) := (List.isChain_cons_cons.1 hchain').2
      have htail' : (b :: tl).Chain' (IsNN (d := d)) := by
        simpa [List.IsChain] using htail
...
private lemma walkTime_dropUntil_le (d : ℕ) (w : Weights d) {u v : Zd d}
  ...
  have htime : walkTime d w p =
    ...
    simpa [hs] using (walkTime_append d w (p.takeUntil x hx) (p.dropUntil x hx))
...
private lemma walkTime_bypass_le (d : ℕ) (w : Weights d) {u v : Zd d}
  ...
  | cons ha p ih =>
      let p' := p.bypass
      by_cases hs : u ∈ p'.support
      ·
        have h1 : walkTime d w (p'.dropUntil u hs) ≤ walkTime d w p' :=
          walkTime_dropUntil_le d w (p := p') (x := u) hs
        have h3 : walkTime d w p ≤ walkTime d w (SimpleGraph.Walk.cons ha p) := by
          simp [walkTime_cons, le_add_of_nonneg_left]
        have : walkTime d w (p'.dropUntil u hs) ≤ walkTime d w p := le_trans h1 (by simpa [p'] using ih)
        -- rewrite bypass in this branch
        simpa [SimpleGraph.Walk.bypass, p', hs] using le_trans this h3
      ·
        have h2' : walkTime d w p' ≤ walkTime d w p := by simpa [p'] using ih
        have h2 : w u v + walkTime d w p' ≤ w u v + walkTime d w p :=
          add_le_add_left h2' (w u v)
        simpa [SimpleGraph.Walk.bypass, p', hs, walkTime_cons, add_comm, add_left_comm, add_assoc] using h2
