...
private lemma walkTime_bypass_le (d : ℕ) (w : Weights d) {u v : Zd d}
  ...
  | cons (u := u0) (v := v0) (w := w0) ha p ih =>
      -- unfold bypass and split on the internal `if`
      simp [SimpleGraph.Walk.bypass]
      split_ifs with hs
      ·
        have h1 : walkTime d w (p.bypass.dropUntil u0 hs) ≤ walkTime d w p.bypass :=
          walkTime_dropUntil_le d w (p := p.bypass) (x := u0) hs
