...
equation.  This is preferable to making an arbitrary global choice of a fixed point.  The main
analytic ingredients of the blueprint are currently stated as lemmas with `sorry`; the final
overlap and free-energy bounds are assembled from those ingredients.
-/
...
/-- Gaussian integration by parts for a linear vector field tested against the gradient of a
smooth function.  This coordinate form is convenient for affine Gaussian interpolations. -/
lemma gaussian_ibp_gradient_linear
    {E K : Type*} [NormedAddCommGroup E] [InnerProductSpace ℝ E] [CompleteSpace E]
    [MeasurableSpace E] [BorelSpace E]
    [NormedAddCommGroup K] [InnerProductSpace ℝ K] [CompleteSpace K]
    [MeasurableSpace K] [BorelSpace K]
    (g : Ω → E) (hg : PhysLean.Probability.GaussianIBP.IsGaussianHilbert g)
    (A B : E →L[ℝ] K) (field : K) (F : K → ℝ)
    (hFi_diff : ∀ i : hg.ι, ContDiff ℝ 1
      (fun x : E => fderiv ℝ F (A x + field) (B (hg.w i))))
    (hFi_growth : ∀ i : hg.ι, PhysLean.Probability.GaussianIBP.HasModerateGrowth
      (fun x : E => fderiv ℝ F (A x + field) (B (hg.w i)))) :
    (∫ w, fderiv ℝ F (A (g w) + field) (B (g w)) ∂ℙ) =
      ∫ w, ∑ i : hg.ι, (hg.τ i : ℝ) *
        fderiv ℝ (fun x : E => fderiv ℝ F (A x + field) (B (hg.w i)))
          (g w) (hg.w i) ∂ℙ := by
  classical
  ...
  have hleft (w : Ω) :
      fderiv ℝ F (A (g w) + field) (B (g w)) =
        ∑ i : hg.ι, PhysLean.Probability.GaussianIBP.coord hg.w g i w * Fi i (g w) := by
    rw [show g w = ∑ i : hg.ι,
      (PhysLean.Probability.GaussianIBP.coord hg.w g i w) • hg.w i by
        simpa [PhysLean.Probability.GaussianIBP.coord, hg.w.repr_apply_apply,
          real_inner_comm] using (hg.w.sum_repr (g w)).symm]
    simp only [map_sum, map_smul, smul_eq_mul, Fi]
  have hLint : ∀ i : hg.ι, Integrable
      (fun w => PhysLean.Probability.GaussianIBP.coord hg.w g i w * Fi i (g w)) ℙ := by
    intro i
    exact PhysLean.Probability.GaussianIBP.integrable_coord_mul_F hg
      (hFi_diff i) (hFi_growth i) i
  have hRint : ∀ i : hg.ι, Integrable
      (fun w => fderiv ℝ (Fi i) (g w) (hg.w i)) ℙ := by
    intro i
    exact PhysLean.Probability.GaussianIBP.integrable_fderiv_apply hg
      (hFi_diff i) (hFi_growth i) (hg.w i)
  calc
    (∫ w, fderiv ℝ F (A (g w) + field) (B (g w)) ∂ℙ) =
        ∑ i : hg.ι, ∫ w,
          PhysLean.Probability.GaussianIBP.coord hg.w g i w * Fi i (g w) ∂ℙ := by
      rw [MeasureTheory.integral_congr_ae (ae_of_all _ hleft)]
      exact MeasureTheory.integral_finset_sum _ (fun i _ => hLint i)
    _ = ∑ i : hg.ι, (hg.τ i : ℝ) *
          ∫ w, fderiv ℝ (Fi i) (g w) (hg.w i) ∂ℙ := by
      apply Finset.sum_congr rfl
      intro i _
      exact PhysLean.Probability.GaussianIBP.ProbabilityTheory.coord_IBP hg
        (hFi_diff i) (hFi_growth i) i
    _ = ∫ w, ∑ i : hg.ι, (hg.τ i : ℝ) *
...
/-- Continuous linear extraction of an affine combination from the joint disorder space. -/
noncomputable def jointAffineCLM (a b : ℝ) :
...
@[simp] lemma jointAffineCLM_apply (a b : ℝ)
    (p : WithLp 2 (EnergySpace N × EnergySpace N)) :
    jointAffineCLM (N := N) a b p =
      a • (WithLp.ofLp p).1 + b • (WithLp.ofLp p).2 := rfl

@[simp] lemma jointAffineCLM_UV (a b : ℝ) (w : Ω) :
    jointAffineCLM (N := N) a b
      (UV (N := N) (β := β) (h := h) (q := q) (sk := sk) (sim := sim) w) =
      a • sk.U w + b • sim.V w := by
  rfl

/-
A free-energy gradient in an affine Gaussian direction is integrable.
-/
lemma integrable_free_energy_gradient_affine
    (a b c d : ℝ) (field : EnergySpace N) :
    Integrable (fun w =>
      fderiv ℝ (fun H : EnergySpace N => free_energy_density (N := N) H)
        (a • sk.U w + b • sim.V w + field) (c • sk.U w + d • sim.V w)) ℙ := by
  refine' MeasureTheory.Integrable.mono' _ _ _;
  refine' fun w => 1 / N * ( |c| * ‖sk.U w‖ + |d| * ‖sim.V w‖ );
  · have h_integrable : Integrable (fun w => ‖sk.U w‖) ℙ ∧ Integrable (fun w => ‖sim.V w‖) ℙ := by
      constructor <;> apply_rules [ PhysLean.Probability.GaussianIBP.integrable_norm_of_gaussian ];
      exact sk.hU;
      exact sim.hV;
    exact MeasureTheory.Integrable.const_mul ( MeasureTheory.Integrable.add ( h_integrable.1.const_mul _ ) ( h_integrable.2.const_mul _ ) ) _;
  · have h_cont : Continuous (fun H : EnergySpace N => fderiv ℝ (fun H => free_energy_density N H) H) := by
      apply_rules [ ContDiff.continuous_fderiv ];
      apply_rules [ ContDiff.log, ContDiff.mul, contDiff_const ];
      exact ⊤;
      · exact ContDiff.sum fun _ _ => ContDiff.exp <| ContDiff.neg <| contDiff_id.comp <| by exact?;
      · exact fun x => ne_of_gt ( Finset.sum_pos ( fun _ _ => Real.exp_pos _ ) Finset.univ_nonempty );
      · decide +revert;
    have h_integrable : AEStronglyMeasurable (fun w => sk.U w) ℙ ∧ AEStronglyMeasurable (fun w => sim.V w) ℙ := by
      have h_integrable : AEStronglyMeasurable (fun w => sk.U w) ℙ := by
        have h_integrable : AEStronglyMeasurable (fun w => sk.U w) ℙ := by
          have h_gauss : HasGaussianLaw (fun w => sk.U w) ℙ := by
            convert gaussianHilbert_hasGaussianLaw sk.hU
          have := h_gauss.aemeasurable;
          exact this.aestronglyMeasurable;
        exact h_integrable
      have h_integrable' : AEStronglyMeasurable (fun w => sim.V w) ℙ := by
        have := sim.hV;
        obtain ⟨ g, hg ⟩ := this;
        simp +decide [ * ];
        fun_prop
      exact ⟨h_integrable, h_integrable'⟩;
    have h_integrable : AEStronglyMeasurable (fun w => a • sk.U w + b • sim.V w + field) ℙ := by
      exact AEStronglyMeasurable.add ( AEStronglyMeasurable.add ( h_integrable.1.const_smul a ) ( h_integrable.2.const_smul b ) ) ( MeasureTheory.aestronglyMeasurable_const );
    have h_integrable : AEStronglyMeasurable (fun w => c • sk.U w + d • sim.V w) ℙ := by
      exact AEStronglyMeasurable.add ( AEStronglyMeasurable.smul ( MeasureTheory.aestronglyMeasurable_const ) ( by tauto ) ) ( AEStronglyMeasurable.smul ( MeasureTheory.aestronglyMeasurable_const ) ( by tauto ) );
    fun_prop (disch := solve_by_elim);
  · -- By definition of $fderiv$, we know that
    have h_fderiv : ∀ H v : EnergySpace N, ‖fderiv ℝ (fun H => free_energy_density N H) H v‖ ≤ (1 / N) * ‖v‖ := by
      intro H v;
      have := fderiv_free_energy_density_apply N H v;
      have h_triangle : |∑ σ : Config N, gibbs_pmf N H σ * v σ| ≤ ∑ σ : Config N, gibbs_pmf N H σ * ‖v‖ := by
        have h_triangle : ∀ σ : Config N, |gibbs_pmf N H σ * v σ| ≤ gibbs_pmf N H σ * ‖v‖ := by
          intro σ
          have h_abs : |v.ofLp σ| ≤ ‖v‖ := by
            exact?;
          rw [ abs_mul, abs_of_nonneg ( gibbs_pmf_nonneg N H σ ) ] ; exact mul_le_mul_of_nonneg_left h_abs ( gibbs_pmf_nonneg N H σ );
        exact le_trans ( Finset.abs_sum_le_sum_abs _ _ ) ( Finset.sum_le_sum fun σ _ => h_triangle σ );
      have h_sum_gibbs_pmf : ∑ σ : Config N, gibbs_pmf N H σ = 1 := by
        exact?;
      simp_all +decide [ ← Finset.sum_mul _ _ _ ];
      exact mul_le_mul_of_nonneg_left h_triangle <| by positivity;
    refine' Filter.Eventually.of_forall fun w => le_trans ( h_fderiv _ _ ) _;
    exact mul_le_mul_of_nonneg_left ( norm_add_le_of_le ( by rw [ norm_smul, Real.norm_eq_abs ] ) ( by rw [ norm_smul, Real.norm_eq_abs ] ) ) ( by positivity )

/-- The SK part of Gaussian integration by parts, with the independent simple disorder
serving as a random shift. -/
lemma sk_gaussian_affine_ibp
...
/-- The simple-disorder part of Gaussian integration by parts, with the independent SK disorder
serving as a random shift. -/
lemma simple_gaussian_affine_ibp
    (hIndep : IndepFun sk.U sim.V (ℙ : Measure Ω))
    ...
    (∫ w,
      ...
      (b * b') * ∫ w, (∑ σ : Config N, ∑ τ : Config N,
        simple_cov_kernel N β (fun x => q * x) σ τ * hessian_free_energy N
          (a • sk.U w + b • sim.V w + field)
          (std_basis N σ) (std_basis N τ)) ∂ℙ := by
  let H : Ω → EnergySpace N := fun w => a • sk.U w + b • sim.V w + field
  let L : Ω → EnergySpace N →L[ℝ] ℝ := fun w =>
    fderiv ℝ (fun X : EnergySpace N => free_energy_density (N := N) X) (H w)
  have hsplit (w : Ω) :
      L w (a' • sk.U w + b' • sim.V w) =
        L w (a' • sk.U w) + L w (b' • sim.V w) := by
    exact map_add (L w) _ _
  have hU : Integrable (fun w => L w (a' • sk.U w)) ℙ := by
    simpa only [H, L, zero_smul, add_zero] using
      integrable_free_energy_gradient_affine
        (N := N) (β := β) (h := h) (q := q) (sk := sk) (sim := sim)
        a b a' 0 field
  have hV : Integrable (fun w => L w (b' • sim.V w)) ℙ := by
    simpa only [H, L, zero_smul, zero_add] using
      integrable_free_energy_gradient_affine
        (N := N) (β := β) (h := h) (q := q) (sk := sk) (sim := sim)
        a b 0 b' field
  change (∫ w, L w (a' • sk.U w + b' • sim.V w) ∂ℙ) = _
  rw [MeasureTheory.integral_congr_ae (ae_of_all _ hsplit),
    MeasureTheory.integral_add hU hV,
    sk_gaussian_affine_ibp
      (N := N) (β := β) (h := h) (q := q) (sk := sk) (sim := sim)
      hIndep a b a' field,
    simple_gaussian_affine_ibp
      (N := N) (β := β) (h := h) (q := q) (sk := sk) (sim := sim)
      hIndep a b b' field]
...
/-- Gaussian interpolation formula for the coupled two-replica free energy. -/
lemma coupled_freeEnergy_derivative_formula
    (hN : 0 < N)
    (hIndep : IndepFun sk.U sim.V (ℙ : Measure Ω))
    ...
    ∃ crossMoment : ℝ,
      ...
      deriv
        (fun s => coupledFreeEnergy
          (N := N) (β := β) (h := h) (q := q) (sk := sk) (sim := sim) s Λ) t =
      (β ^ 2 / 4) *
        ((1 - q) ^ 2 +
          4 * deriv
...
/-
Derivative identity for the coupled interpolation.
-/
lemma coupled_excess_derivative
    (hN : 0 < N)
    (hIndep : IndepFun sk.U sim.V (ℙ : Measure Ω))
    {t Λ : ℝ} (ht : t ∈ Set.Ioo (0 : ℝ) 1) :
    (∃ crossMoment : ℝ,
...
theorem overlap_concentration_uniform
  ...
  have h2 : logQuadraticMoment (N := N) (β := β) (h := h) (q := q) (sk := sk) (sim := sim) t (lambdaStar β q) ≤ quadraticConstant β q :=
    uniform_quadratic_coupling (hN := hN) (hβ := hβ) (hq0 := hq0) (hq1 := hq1)
...
theorem generalized_latala
  ...
  have hoverlap :=
    overlap_concentration_uniform
      (N := N) (β := β) (h := h) (q := q) (sk := sk) (sim := sim)
      hN hβ hq0 hq1 hfp hρ hIndep (t := (1 : ℝ)) (by simp)
  have hsum :=
  ...
  have hbound : ∀ t ∈ Set.Icc (0 : ℝ) 1,
    ...
    simpa [C] using
      overlap_concentration_uniform
        (N := N) (β := β) (h := h) (q := q) (sk := sk) (sim := sim)
        hN hβ hq0 hq1 hfp hρ hIndep ht
